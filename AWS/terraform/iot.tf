...
resource "aws_iot_topic_rule" "davis_rule" {
  name        = "${var.project}DavisRuleToDynamo"
  description = "ingest incoming davis data into weather data table"
  enabled     = true
  sql         = "SELECT value FROM '${var.davis_topic}'"
...
resource "aws_iam_role_policy" "iot_allow_dynamo_davis_policy" {
  name = "${var.project}-iot-dynamo-davis-policy"
  role = aws_iam_role.iot_davis_role.id
